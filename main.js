...
const Config = {
	...
	scriptureNoChpRegex: /(('?)(phm|philemon|(?:2|3)(?: )?jo(?:hn)?|jud(?:e)?)(?: ?)(\d{1,2}))(\]|<\/a>)?/gim,
...
class JWLLinkerPlugin extends Plugin {
	...
	async insertCitation(editor, command, pars = 0) {
		...
		const activeEditor = this.confirmEditor(editor);
		if (!activeEditor) return;
		let { selection, caret, line } = this._getEditorSelection(activeEditor, true);
		if (selection) {
	...
	async _fetchParagraphCitation(input, view, caret, command, pars) {
		const failure = {
			output: input,
			changed: false,
			error: OutputError.onlineLookupFailed,
		};
	...
	_getAllScriptureLinks(input, displayType, spaceAfterPunct = false, caret = undefined) {
		...
		function validateReference(reference, displayType) {
			...
			for (const passage of reference.passages) {
				...
				if (
					bookAndChapter in BibleDimensions &&
...
class ScripturePostProcessor extends MarkdownRenderChild {
	...
	onload() {
		const { output, changed } = this.plugin._convertScriptureToJWLibrary(this.containerEl.innerHTML, DisplayType.url);
		if (changed) {
...
class JWLLinkerSettingTab extends PluginSettingTab {
	...
	display() {
		...
		new Setting(containerEl)
			...
			.setDesc('Use this template when citing a short inline excerpt from a scripture or a publication.')
			.addTextArea((text) => {
				text
					.setPlaceholder(defaultTemplate)
